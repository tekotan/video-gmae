...
"""
python /home/tekotan/Vidgmae_test/vidgmae/utils/generate_tapvid.py \
  --input_csv_path=/home/tekotan/Vidgmae_test/data-download/tapvid_kinetics/tapvid_kinetics.csv \
  --output_base_path=. \
  --video_root_path=/home/tekotan/Vidgmae_test/data-download/kinetics/kinetics-dataset/k700-2020/val/ \
  --alsologtostderr
"""
import csv
import dataclasses
...
def csv_to_dataset(csv_path: str, videos_path: Dict[str, str]) -> Tuple[Video, ...]:
...
def generate_example(video: Video) -> Dict[str, np.ndarray]:
    ...
    if len(imgs_encoded) < 250:
        ...
        new_tracks = tuple(Track(tuple(t.points[:num_frames])) for t in video.tracks)
        video = Video(
            video.youtube_id,
