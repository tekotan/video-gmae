...
from vidgmae.utils.model_utils import (
    add_weight_decay,
...
def compute_tapvid_metrics_length_scaled(
    ...
    *_per_frame metrics that are length-agnostic.
    ...
    for thresh in [1, 2, 4, 8, 16]:
        ...
        false_pos = np.sum(
            ((~visible) | (~within_dist)) & pred_visible & evaluation_points,
            axis=(1, 2),
        )
    ...
    metrics["average_pts_within_thresh_per_frame"] = np.mean(
        np.stack(
            [metrics[f"pts_within_{t}_per_frame"] for t in [1, 2, 4, 8, 16]], axis=1
        ),
        axis=1,
    )
    return metrics
...
class FinetuneLitModule(LightningModule):
    ...
    def __init__(
        ...
        import vidgmae.models.components.mae.models_mae_finetune as models_mae
    ...
    def forward_loss(self, true, pred, additional_data=None):
        ...
        for i in range(B):
            ...
            if self.mode == "point-tracking":
                if self.cfg.finetune_params.zero_t_prediction:
                    tracks_true = torch.cat(
                        [true[0][:, :, 0:1], true[0][:, :, frame_num : frame_num + 1]],
                        dim=2,
                    )
                    occluded_true = torch.cat(
                        [true[1][:, :, 0:1], true[1][:, :, frame_num : frame_num + 1]],
                        dim=2,
                    )
                else:
                ...
                if self.cfg.finetune_params.quantized_prediction:
                    quantized_targets = (
                        torch.round(
                            active_tracks_true
                            * self.cfg.finetune_params.quantize_output_bins
                        )
                        .type(torch.int64)
                        .clip(
                            min=0, max=self.cfg.finetune_params.quantize_output_bins - 1
                        )
                    )
                        ...
                        for k in range(active_tracks_true.shape[2]):
                            loss += (
                                torch.nn.functional.cross_entropy(
                                    active_tracks_pred[:, j, k],
                                    quantized_targets[:, j, k],
                                )
                                * 100
                            )
                    ...
                    loss += (
                        torch.nn.functional.binary_cross_entropy(
                            active_occluded_pred.float(), active_occluded_true.float()
                        )
                        * 0.1
                    )
                    ...
                    loss += (
                        torch.nn.functional.mse_loss(
                            active_tracks_true, active_tracks_pred
                        )
                        * 100
                    )
                    loss += (
                ...
                if self.cfg.finetune_params.zero_t_prediction:
                    target_boxes = torch.cat(
                        [true[:, :, 0:1], true[:, :, frame_num : frame_num + 1]], dim=2
                    )
                    pred_boxes = pred
                else:
                ...
                for j in range(len(target_boxes)):
                    ...
                    if self.cfg.finetune_params.quantized_prediction:
                        quantized_targets = (
                            ...
                            .clip(
                                min=0,
                                max=self.cfg.finetune_params.quantize_output_bins - 1,
                            )
                        )
                        for j in range(target_boxes_clone.shape[1]):
                            loss += (
                                torch.nn.functional.cross_entropy(
                                    pred_boxes_clone[:, j], quantized_targets[:, j]
                                )
                                * 100
                            )
                    elif self.cfg.finetune_params.dit_head:
                        loss += torch.nn.functional.mse_loss(
                            pred_boxes_clone, target_boxes_clone
                        )
                    else:
                        iou_loss = complete_box_iou_loss(
                            pred_boxes_clone, target_boxes_clone, reduction="mean"
                        )
    ...
    def step(self, batch: Any, batch_idx: int, return_images=False):
        ...
        if self.mode == "point-tracking":
            x_points, occlusions_pred = self.encoder.forward_decoder(
                latent, ids_restore, init_queries
            )  # B x 256 x T x 2
            loss = self.forward_loss(
                (finetune_target, occluded_points),
                (x_points, occlusions_pred),
                additional_data=init_queries,
            )

        elif self.mode == "object-tracking":
            box_pred = self.encoder.forward_decoder(
                latent, ids_restore, init_queries
            )  # B x 256 x T x 2
            loss = self.forward_loss(
                finetune_target, box_pred, additional_data=init_queries
            )
            ...
            with torch.no_grad():
                ...
                if self.mode == "point-tracking":
                    ...
                    if self.cfg.inference.save_predictions:
                        ...
                        torch.save(
                            data,
                            f"{self.cfg.storage_folder}/tests/eval_{self.cfg.inference.context_length}_{self.current_epoch}_{batch_idx}.pt",
                        )
                    ...
                    final_image_ = self.visualize_object_tracking(
                        video, box_pred, finetune_target, batch_idx
                    )
                    ...
                    if self.cfg.inference.save_predictions:
                        ...
                        torch.save(
                            data,
                            f"{self.cfg.storage_folder}/tests/finetune_{self.cfg.inference.context_length}_{self.current_epoch}_{batch_idx}.pt",
                        )
        ...
        if self.mode == "point-tracking":
            extras_ = {
                "x_points": x_points,
                "occluded_pred": occlusions_pred,
            }  # {"loss_index": loss_index.mean(dim=0)}
        elif self.mode == "object-tracking":
    ...
    def visualize_object_tracking(self, video, pred_boxes, target_boxes, batch_idx):
        ...
        for b in range(len(imgs)):
            ...
            for t in range(len(imgs[b])):
                ...
                for pred_box, target_box in zip(
                    pred_boxes[b, :, t].cpu().numpy(),
                    target_boxes[b, :, t].cpu().numpy(),
                ):
                    # Draw ground truth box
                    ymin, xmin, ymax, xmax = (
                        target_box * self.cfg.input_size
                    )  # Scale to image size
                    if ymin < 0 or xmin < 0:
                        continue
                    cv2.rectangle(
    ...
    def visualize_point_tracking(
        ...
        for i in range(len(x_points)):
            ...
            active_tracks_pred = (
                x_points[i, :first_invalid_index].detach().cpu().numpy()
            )
            active_tracks_true = (
                target_points[i, :first_invalid_index].detach().cpu().numpy()
            )

            active_occluded_points = (
                occluded_points[i, :first_invalid_index].detach().cpu().numpy()
            )
            active_occlusions_pred = (
                occlusions_pred[i, :first_invalid_index].detach().cpu().numpy()
            )
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        ...
        if self.mode == "point-tracking":
            ...
            self.test_mse.update(
                target_points.contiguous(), filtered_x_points.contiguous()
            )

            # Update Jaccard metric
            pred_points = filtered_x_points
            pred_visible = ~(
                extra.get("occluded_pred", torch.zeros_like(query_points[:, :, 0]))
                > 0.5
            ).squeeze(-1)
            ...
            for i in range(query_points.shape[0]):
                ...
                pred_points_ = (
                    pred_points[i : i + 1, :first_invalid_index, :first_n_frames]
                    * self.cfg.input_size
                )
                ...
                if set(
                    (1 - (~true_visible_).detach().cpu().numpy()).flatten().tolist()
                ) == {0}:
                    log.info(
                ...
                metrics = compute_tapvid_metrics(
                    query_points_.detach().cpu().numpy(),
                    (~true_visible_).detach().cpu().numpy(),
                    true_points_.detach().cpu().numpy(),
                    (~pred_visible_).detach().cpu().numpy(),
                    pred_points_.detach().cpu().numpy(),
                )
        ...
        if self.mode == "object-tracking":
            ...
            for i in range(query_points.shape[0]):
                ...
                pred_boxes_ = (
                    pred_boxes[i : i + 1, :first_invalid_index] * self.cfg.input_size
                )
                # pred_visible_ = pred_visible[i:i+1, :first_invalid_index]

                true_boxes_ = (
                    target_boxes[i : i + 1, :first_invalid_index] * self.cfg.input_size
                )
    ...
    def on_validation_epoch_end(self):
        ...
        if (
            self.cfg.task == "finetune"
            or "imagenet" in self.cfg.training_type
            or "k400" in self.cfg.training_type
            or "ucf101" in self.cfg.training_type
        ):
            if self.mode == "point-tracking":
                log.info(
                    "Jaccard Score : " + str(self.average_jaccard.compute().item())
                )
                log.info("Avg Distance : " + str(self.avg_distance.compute().item()))
                log.info(
                    "Avg Points within Thresh : "
                    + str(self.average_pts_within_thresh.compute().item())
                )
                log.info(
                    "Occlusion Accuracy : "
                    + str(self.occlusion_accuracy.compute().item())
                )
