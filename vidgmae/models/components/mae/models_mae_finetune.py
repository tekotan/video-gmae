...
from vidgmae.models.components.mae.models_mae_video import MaskedAutoencoderViT
from vidgmae.models.components.dit.dit_gaussian import TimestepEmbedder, DiT, FinalLayer
...
class CrossAttentionReadout(nn.Module):
    def __init__(
        ...
        # Temporal embeddings
        self.enc_temporal_embed = nn.Parameter(
            torch.zeros(1, num_enc_tokens, embed_dim)
        )
        self.query_temporal_embed = nn.Parameter(
            torch.zeros(1, num_input_frames, embed_dim)
        )
        ...
        fourier_dim = (
            self.num_fourier_features * cond_size
        )  # 3 for xyz coords, *2 for sin/cos
...
class FinetuneMaskedAutoencoderViT(MaskedAutoencoderViT):
    def __init__(
        ...
        new_readout_mode=True,
        zero_t_prediction=False,
        autoregressive=False,
        quantized_prediction=False,
        quantize_output_bins=None,
        dit_head=False,
        ...
        enc_tokens = (
            num_patches * self.input_frames // 2
            if self.videomae or self.mae_st
            else num_patches * self.input_frames
        )
        ...
        if self.mode == "point-tracking":
            ...
            self.cond_size = (
                self.num_fourier_features * self.output_size
        ...
        if self.videomae:
            ...
            if "interpolate" in self.training_type:
                ...
                x_ = (
                    x_
                    + self.pos_embed_t[
                        :, : self.input_frames * self.patch_embed.num_patches, None, :
                    ]
                )
                #
                ...
                x = (
                    x_
                    + self.pos_embed[
                        :, : self.input_frames * self.patch_embed.num_patches
                    ]
                )
...
finetune_vit_base_patch16 = (
    finetune_vit_base_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
finetune_vit_base_patch16_small = (
    finetune_vit_base_patch16_dec512d2b  # decoder: 512 dim, 4 blocks
)
finetune_vit_large_patch16 = (
    finetune_vit_large_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
finetune_vit_huge_patch14 = (
    finetune_vit_huge_patch14_dec512d8b  # decoder: 512 dim, 8 blocks
)
finetune_vit_huge_patch16 = (
    finetune_vit_huge_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
finetune_vit_huge_patch14_JM = (
    finetune_vit_huge_patch14_dec512d8b_JM  # decoder: 512 dim, 8 blocks
)


finetune_vit_giga_patch16 = (
    finetune_vit_giga_patch16_dec512d8b  # decoder: 512 dim, 8 blocks
)
