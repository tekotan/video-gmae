...
def random_crop_4d(
    ...
    # 1) Crop the frames
    cropped_frames = frames_4d[
        :, :, y_off : y_off + crop_size, x_off : x_off + crop_size
    ]
    ...
    external_x = (adjusted_points[:, :, 0] < 0) | (
        adjusted_points[:, :, 0] >= crop_size
    )
    external_y = (adjusted_points[:, :, 1] < 0) | (
        adjusted_points[:, :, 1] >= crop_size
    )
...
class PointTrackingEvalDataset(IterableDataset):
    def __init__(self, pickle_files, cfg):  # List[str]
        super().__init__()
        self.cfg = cfg
                ...
                for video_name, content in data.items():
                    if "eval-kinetics" in self.cfg.training_type:
                        resized_video = resize_video(
                            content["video"], nearest_power_of_two
                        )
                        del content["video"]
            ...
            while start_idx < T:
                ...
                if N < self.cfg.finetune_params.tracks_to_sample:
                    ...
                    pad_target = torch.zeros(
                        diff, length, 2, dtype=target_points_torch.dtype
                    )
                    target_points_torch = torch.cat(
                        [target_points_torch, pad_target], dim=0
                    )
                    # query_points => pad with -1
                    pad_query = -1.0 * torch.ones(
                        diff, 3, dtype=query_points_torch.dtype
                    )
                    query_points_torch = torch.cat(
                        [query_points_torch, pad_query], dim=0
                    )
                    # occluded_points => pad with 1
                    pad_occluded = torch.ones(
                        diff, length, dtype=occluded_points_torch.dtype
                    )
                ...
                yield (
                    frames_chunk.float(),
                    query_points_torch.float(),
                    target_points_torch.float(),
                    occluded_points_torch.float(),
                )
