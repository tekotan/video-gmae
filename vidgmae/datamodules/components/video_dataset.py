...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if num_frames > self.cfg.seq_length * self.cfg.sample_rate:
            start = (
                np.random.randint(
                    0, num_frames - self.cfg.seq_length * self.cfg.sample_rate
                )
                if self.cfg.random_clip
                else 0
            )
            frames = frames[
                start : start
                + self.cfg.seq_length * self.cfg.sample_rate : self.cfg.sample_rate
            ]
        ...
        try:
            ...
            frames_ = np.random.randint(
                0,
                255,
                (self.cfg.seq_length, self.cfg.input_size, self.cfg.input_size, 3),
                dtype=np.uint8,
            )
        ...
        for i, frame in enumerate(frames_):
            ...
            arr = arr[
                crop_y : crop_y + self.cfg.input_size,
                crop_x : crop_x + self.cfg.input_size,
            ]
        ...
        if arr_seq.shape[1] < self.cfg.seq_length:
            ...
            fill = np.zeros(
                (3, required_dim, self.cfg.input_size, self.cfg.input_size),
                dtype=np.float32,
            )
