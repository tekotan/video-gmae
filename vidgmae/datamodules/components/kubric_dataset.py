...
def project_point(cam, point3d, num_frames):
    ...
    image_coords = tf.concat(
        [image_coords[:, :, :2], tf.sign(projected[:, :, 2:])], axis=2
    )
    return image_coords
...
def reproject(coords, camera, camera_pos, num_frames, bbox=None):
    ...
    if bbox is not None:
        ...
        world_coords = tf.matmul(
            tf.cast(
                tf.concat([coords, tf.ones_like(coords[:, 0:1])], axis=1), tf.float32
            )[tf.newaxis, :, :],
            local_to_world,
        )
...
def get_num_to_sample(counts, max_seg_id, max_sampled_frac, tracks_to_sample):
    ...
    def scan_fn(prev_output, count_seg):
        ...
        want_to_sample = tf.cast(remaining_needed, tf.float32) * tf.cast(
            desired_frac, tf.float32
        )
        want_to_sample = tf.cast(tf.round(want_to_sample), tf.int32)
        max_to_sample = tf.cast(count_seg, tf.float32) * tf.cast(
            max_sampled_frac, tf.float32
        )
...
def track_points(
    ...
    cnt = tf.math.bincount(tf.cast(tf.reshape(segmentations_box, [-1]) + 1, tf.int32))[
        1:
    ]
    ...
    for i in range(max_seg_id):
        ...
        if obj_id == -1:
            ...
            bbox = tf.cond(
                obj_id >= tf.shape(bboxes_3d)[0],
                lambda: bboxes_3d[0, :],
                lambda: bboxes_3d[obj_id, :],
            )
            quat = tf.cond(
                obj_id >= tf.shape(obj_quat)[0],
                lambda: obj_quat[0, :],
    ...
    wd = tf.concat(
        [np.array([0.0]), window[0:2], np.array([num_frames]), window[2:4]], axis=0
    )
...
class KubricPointTrackingDataset(IterableDataset):
    def __init__(self, cfg, train=True, tracks_to_sample=256, **kwargs):
        super().__init__()
        self.cfg = cfg
        self.train = train
        ...
        with tf.device("/CPU:0"):
            ...
            #    This will consume the dataset in an iterator-like fashion.
    ...
    def __iter__(self):
        for sample in self.generator:
            ...
            target_points_np = sample["target_points"][
                :, :num_frames
            ]  # shape: (256, 24, 2)
            occluded_points_np = sample["occluded"].astype(float)[
                :, :num_frames
            ]  # shape: (256, 24)
            ...
            video_np = (
                video_np - np.array([0.485, 0.456, 0.406], dtype=np.float32)
            ...
            if N < self.cfg.finetune_params.tracks_to_sample:
                ...
                pad_target = torch.zeros(
                    diff, target_points_np.shape[1], 2, dtype=target_points_torch.dtype
                )
                target_points_torch = torch.cat(
                    [target_points_torch, pad_target], dim=0
                )
                ...
                pad_occluded = torch.ones(
                    diff,
                    occluded_points_torch.shape[1],
                    dtype=occluded_points_torch.dtype,
                )
                occluded_points_torch = torch.cat(
                    [occluded_points_torch, pad_occluded], dim=0
                )

            if self.cfg.finetune_params.batch_prediction:
                num_delete = np.random.randint(
                    0, self.cfg.finetune_params.tracks_to_sample
                )
