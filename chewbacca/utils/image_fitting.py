...
from chewbacca.models.components.mae.models_mae_video import mae_vit_base_patch16 
...
class SimpleTrainer:
    ...
    def __init__(
        ...
        self.model = mae_vit_base_patch16(number_of_frames=2, num_gaussian=num_points).cuda()
    ...
    def train(
        ...
        for iter in range(iterations):
            start = time.time()
            loss, pred, mask, latent, latents_layer = self.model.forward(self.gt_image.permute(2, 0, 1).unsqueeze(1).unsqueeze(0).repeat(2, 1, 2, 1, 1), mask_ratio=0.0)
