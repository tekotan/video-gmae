# !/bin/bash
# seq_len=2
python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_base_seq2_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=1e-4 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=8 \
configs.test_batch_size=8 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=2 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=1000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_base_k400_test1/0/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=8

# seq_len=4
python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_base_seq4_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=1e-4 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=8 \
configs.test_batch_size=8 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=4 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=1000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_base_k400_test1/1/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=8

# seq_len=4
python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_base_seq24_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=1e-4 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=8 \
configs.test_batch_size=8 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=24 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=1000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_base_k400_test1/4/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=8

# Mean false RGB true
python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_base_seq8_mFrT_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=1e-4 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=8 \
configs.test_batch_size=8 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=8 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=1000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_large_k400_test7/1/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=8

# Mean true RGB false
python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_base_seq8_mTrF_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=1e-4 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=8 \
configs.test_batch_size=8 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=8 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=1000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_large_k400_test7/2/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=8

# Mean false RGB false
python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_base_seq8_mFrF_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=1e-4 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=8 \
configs.test_batch_size=8 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=8 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=1000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_large_k400_test7/3/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=8

# # !/bin/bash
# python chewbacca/validate.py -m \
# --config-name gmae_ema.yaml \
# model._target_="chewbacca.models.finetune.FinetuneLitModule" \
# datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
# task_name=vgmae_base_seq16_finetune_point_v1 \
# trainer=ddp_unused \
# trainer.devices=1 \
# trainer.num_nodes=1 \
# configs.task="finetune" \
# configs.model_name="finetune_vit_base_patch16" \
# configs.input_size=224 \
# configs.lr=1e-4 \
# configs.weight_decay=5e-2 \
# trainer.accumulate_grad_batches=1 \
# configs.train_batch_size=8 \
# configs.test_batch_size=8 \
# configs.train_num_workers=1 \
# configs.test_num_workers=1 \
# trainer.gradient_clip_val=2.0 \
# configs.scheduler="cosine_step" \
# configs.lr_interval="step" \
# configs.warmup_steps=20 \
# trainer.max_epochs=200 \
# configs.seq_length=16 \
# configs.num_classes=400 \
# configs.dataset_type="video" \
# trainer.limit_train_batches=1000 \
# trainer.limit_val_batches=100 \
# callbacks.model_checkpoint.every_n_epochs=1 \
# configs.training_type="eval-davis_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
# configs.load_strict=False \
# configs.mask_ratio=0.0 \
# configs.finetune_params.num_fourier_features=64 \
# configs.finetune_params.new_readout_mode=True \
# configs.finetune_params.tracks_to_sample=16 \
# configs.inference.testing=True \
# configs.inference.context_length=1 \
# configs.inference.save_predictions=True