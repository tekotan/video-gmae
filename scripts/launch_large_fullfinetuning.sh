# # !/bin/bash
# python chewbacca/train.py -m \
# --config-name gmae_ema.yaml \
# model._target_="chewbacca.models.finetune.FinetuneLitModule" \
# datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
# task_name=vgmae_large_seq16_fullfinetune_point_new \
# trainer=ddp_unused \
# trainer.devices=1 \
# trainer.num_nodes=1 \
# configs.task="finetune" \
# configs.model_name="finetune_vit_large_patch16" \
# configs.input_size=224 \
# configs.lr=5e-5 \
# configs.weight_decay=5e-2 \
# trainer.accumulate_grad_batches=1 \
# configs.train_batch_size=4 \
# configs.test_batch_size=4 \
# configs.train_num_workers=1 \
# configs.test_num_workers=1 \
# trainer.gradient_clip_val=2.0 \
# configs.scheduler="cosine_step" \
# configs.lr_interval="step" \
# configs.warmup_steps=20 \
# trainer.max_epochs=50 \
# configs.seq_length=16 \
# configs.num_classes=400 \
# configs.dataset_type="video" \
# trainer.limit_train_batches=1000 \
# trainer.limit_val_batches=100 \
# callbacks.model_checkpoint.every_n_epochs=1 \
# configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
# configs.weights_path="./logs2/vgmae_large_k400_test4/0/checkpoints/last.ckpt" \
# configs.load_strict=False \
# configs.mask_ratio=0.0 \
# configs.finetune_params.num_fourier_features=64 \
# configs.finetune_params.new_readout_mode=True \
# configs.finetune_params.freeze_encoder=False \
# configs.finetune_params.tracks_to_sample=16

# # !/bin/bash
# python chewbacca/train.py -m \
# --config-name gmae_ema.yaml \
# model._target_="chewbacca.models.finetune.FinetuneLitModule" \
# datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
# task_name=vit_large_videomae_fullfinetune_point_tracking_crossattn_v2 \
# trainer=ddp_unused \
# trainer.devices=1 \
# trainer.num_nodes=1 \
# configs.task="finetune" \
# configs.model_name="finetune_vit_large_patch16" \
# configs.input_size=224 \
# configs.lr=5e-5 \
# configs.weight_decay=5e-2 \
# trainer.accumulate_grad_batches=1 \
# configs.train_batch_size=4 \
# configs.test_batch_size=4 \
# configs.train_num_workers=1 \
# configs.test_num_workers=1 \
# trainer.gradient_clip_val=2.0 \
# configs.scheduler="cosine_step" \
# configs.lr_interval="step" \
# configs.warmup_steps=20 \
# trainer.max_epochs=50 \
# configs.seq_length=16 \
# configs.num_classes=200 \
# configs.dataset_type="video" \
# trainer.limit_train_batches=1000 \
# trainer.limit_val_batches=100 \
# callbacks.model_checkpoint.every_n_epochs=1 \
# configs.training_type="train_ucf101_point-tracking_save-images_no-mask_random-frames" \
# configs.load_strict=False \
# configs.mask_ratio=0.0 \
# configs.videomae="large" \
# configs.mae_st=False \
# configs.finetune_params.num_fourier_features=64 \
# configs.finetune_params.new_readout_mode=True \
# configs.finetune_params.tracks_to_sample=16 \
# configs.finetune_params.freeze_encoder=False

# # !/bin/bash
# python chewbacca/train.py -m \
# --config-name gmae_ema.yaml \
# model._target_="chewbacca.models.finetune.FinetuneLitModule" \
# datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
# task_name=vit_large_mae_st_fullfinetune_point_tracking_crossattn_new \
# trainer=ddp_unused \
# trainer.devices=1 \
# trainer.num_nodes=1 \
# configs.task="finetune" \
# configs.model_name="finetune_vit_large_patch16" \
# configs.input_size=224 \
# configs.lr=5e-5 \
# configs.weight_decay=5e-2 \
# trainer.accumulate_grad_batches=1 \
# configs.train_batch_size=4 \
# configs.test_batch_size=4 \
# configs.train_num_workers=1 \
# configs.test_num_workers=1 \
# trainer.gradient_clip_val=2.0 \
# configs.scheduler="cosine_step" \
# configs.lr_interval="step" \
# configs.warmup_steps=20 \
# trainer.max_epochs=50 \
# configs.seq_length=16 \
# configs.num_classes=200 \
# configs.dataset_type="video" \
# trainer.limit_train_batches=1000 \
# trainer.limit_val_batches=100 \
# callbacks.model_checkpoint.every_n_epochs=1 \
# configs.training_type="train_ucf101_point-tracking_save-images_no-mask_random-frames" \
# configs.load_strict=False \
# configs.mask_ratio=0.0 \
# configs.mae_st="large" \
# configs.videomae=False \
# configs.finetune_params.num_fourier_features=64 \
# configs.finetune_params.new_readout_mode=True \
# configs.finetune_params.tracks_to_sample=16 \
# configs.finetune_params.freeze_encoder=False

# # !/bin/bash
# python chewbacca/train.py -m \
# --config-name gmae_ema.yaml \
# model._target_="chewbacca.models.finetune.FinetuneLitModule" \
# datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
# task_name=vit_large_videomae_fullfinetune_point_tracking_crossattn_new \
# trainer=ddp_unused \
# trainer.devices=1 \
# trainer.num_nodes=1 \
# configs.task="finetune" \
# configs.model_name="finetune_vit_large_patch16" \
# configs.input_size=224 \
# configs.lr=5e-5 \
# configs.weight_decay=5e-2 \
# trainer.accumulate_grad_batches=1 \
# configs.train_batch_size=4 \
# configs.test_batch_size=4 \
# configs.train_num_workers=1 \
# configs.test_num_workers=1 \
# trainer.gradient_clip_val=2.0 \
# configs.scheduler="cosine_step" \
# configs.lr_interval="step" \
# configs.warmup_steps=20 \
# trainer.max_epochs=50 \
# configs.seq_length=16 \
# configs.num_classes=200 \
# configs.dataset_type="video" \
# trainer.limit_train_batches=1000 \
# trainer.limit_val_batches=100 \
# callbacks.model_checkpoint.every_n_epochs=1 \
# configs.training_type="train_ucf101_point-tracking_save-images_no-mask_random-frames" \
# configs.load_strict=False \
# configs.mask_ratio=0.0 \
# configs.videomae="large" \
# configs.mae_st=False \
# configs.finetune_params.num_fourier_features=64 \
# configs.finetune_params.new_readout_mode=True \
# configs.finetune_params.tracks_to_sample=16 \
# configs.finetune_params.freeze_encoder=False

python chewbacca/train.py -m \
--config-name gmae_ema.yaml \
model._target_="chewbacca.models.finetune.FinetuneLitModule" \
datamodule._target_="chewbacca.datamodules.finetune_datamodule.FinetuneDataModule" \
task_name=vgmae_large_seq24_finetune_point_v1 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
configs.task="finetune" \
configs.model_name="finetune_vit_large_patch16" \
configs.input_size=224 \
configs.lr=7e-5 \
configs.weight_decay=5e-2 \
trainer.accumulate_grad_batches=1 \
configs.train_batch_size=4 \
configs.test_batch_size=4 \
configs.train_num_workers=1 \
configs.test_num_workers=1 \
trainer.gradient_clip_val=2.0 \
configs.scheduler="cosine_step" \
configs.lr_interval="step" \
configs.warmup_steps=20 \
trainer.max_epochs=50 \
configs.seq_length=24 \
configs.num_classes=400 \
configs.dataset_type="video" \
trainer.limit_train_batches=2000 \
trainer.limit_val_batches=100 \
callbacks.model_checkpoint.every_n_epochs=1 \
configs.training_type="train_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
configs.weights_path="./logs2/vgmae_large_k400_test6/0/checkpoints/last.ckpt" \
configs.load_strict=False \
configs.mask_ratio=0.0 \
configs.finetune_params.num_fourier_features=64 \
configs.finetune_params.new_readout_mode=True \
configs.finetune_params.freeze_encoder=True \
configs.finetune_params.tracks_to_sample=16