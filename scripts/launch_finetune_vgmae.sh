...
task_name=vgmae_large_kinetics_fullfinetune_point_tracking_crossattn_v2 \
trainer=ddp_unused \
trainer.devices=1 \
trainer.num_nodes=1 \
...
configs.weights_path="logs/vgmae_large_k400_test2/0/checkpoints/last.ckpt" \
...
# trainer.limit_val_batches=100 \
# callbacks.model_checkpoint.every_n_epochs=1 \
# configs.training_type="eval_ucf101_point-tracking_save-images_no-mask_interpolate-pos-emb" \
# configs.load_strict=False \
# configs.mask_ratio=0.0 \
# configs.finetune_params.num_fourier_features=64 \
# configs.finetune_params.new_readout_mode=True \
